...
class ViT(nn.Module):
    ...
    def forward(self, x_list):
        ...
        print("pos_embed shape input 1", pose_embed.shape)
        if self.pos_embed.shape[1] != ws[0] * ws[1] * ws[2]:
            # pos_embed=[1 8*14*14 384]->[1 8*16*29 384]
            pos_embed = pos_embed.reshape(ws[0], -1, C)
            my_grid_size = torch.tensor(self.grid_size).to('cuda')
            ws_tensor = torch.tensor([ws[1], ws[2]]).to('cuda')
            num_extra_tokens_tensor = torch.tensor(0).to('cuda')
            print("pos_embed shape input 2", pose_embed.shape)
            print("my_grid_size", my_grid_size)
            print("ws_tensor", ws_tensor)

            pos_embed = interpolate_pos_embed_online(
                pos_embed, my_grid_size, ws_tensor, num_extra_tokens_tensor).reshape(1, -1, C)
            print("pos_embed shape output", pos_embed.shape)
